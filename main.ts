...
class GetTypeGenerator {
  ...
  private isPrimitiveOrNull(v: unknown): boolean {
   ...
   * @returns typeof v
   */
  private getPrimitiveTypeString(v: any): string {
  ...
  private handleFunctionType(fn: Function): string {
    ...
    if (fn_arguments_RegExp.test(fn_str)) {
      const fn_type = (fn_arguments_RegExp.exec(fn_str) as RegExpExecArray)[0];
   ...
   */
  private markAndCheckCircular(obj: object): boolean {
 ...
 */\ninterface ${InterfaceName ?? "RootType"} {`;
  ...
  private tryMutateInterfaceStr(code: string, mutate: (code: string) => void) {
  ...
  private interpretHandlerData(
    Data: EventHandlerReturn,
    mutate: (code: string) => void
  ): { action: "none" | "continue" | "return"; returnValue?: string } {
    if (Data === FnActions.None) return { action: "none" };
    if (Data === FnActions.Continue) return { action: "continue" };
    if (!Array.isArray(Data)) return { action: "none" };

    const [act, payload] = Data;
    if (act === FnActions.Return) {
      return { action: "return", returnValue: payload };
    }
    if (act === FnActions.Eval) {
  ...
  private runTopHandlersAndMaybeMutateInterfaceStr(
    ...
    for (const Data of this.runHandlers(
  ...
  private applyReturnHandlers(
    ...
    for (const Data of this.runHandlers(
      ...
      if (Array.isArray(Data) && Data[0] === FnActions.SetReturn) {
        interfaceStr = Data[1];
      }
    }
    return interfaceStr;
  ...
  public generate(obj: GetType_obj_type, InterfaceName?: string): string {
    ...
    if (primitiveHandled) return this.finalizeEarly(primitiveHandled);

    // 循環引用
    if (typeof obj === "object" && obj) {
      if (this.markAndCheckCircular(obj)) {
        return this.finalizeEarly(
          "any" + (this.config.printHint ? "/* circular */" : "")
        );
      }
    }
    ...
    try {
      for (const key of Object.keys(obj as any)) {
        ...
        if (r.earlyReturn) return this.finalizeEarly(r.earlyReturn);
        if (r.skip) continue;

        interfaceStr = r.interfaceStr;
        if (r.isArrayKey) isArray = true;
      }
    } finally {
      if (prep.safeWindow && !prep.safeWindow.closed) prep.safeWindow.close();
    }
  ...
  // 新增：早期返回統一出口
  private finalizeEarly(val: string): string {
    this.generate_back();
    return val;
  }
   ...
   */
  private tryHandlePrimitiveOrFunction(obj: GetType_obj_type): string | null {
    if (this.isPrimitiveOrNull(obj)) return this.getPrimitiveTypeString(obj);
    if (typeof obj === "function") return this.handleFunctionType(obj);
    return null;
  }
  ...
  private prepareInterface(
    ...
    interfaceStr: string;
    safeWindow: (Window & { [k: string]: any }) | null;
  ...
  private processKey(params: {
    ...
    const { key, obj, InterfaceName, obj_isWindow, safeWindow, mutate } =
      params;
    ...
    // 頂層事件處理
    const handlerResult = this.runTopHandlersAndMaybeMutateInterfaceStr(
      key,
      element,
      obj,
      InterfaceName,
